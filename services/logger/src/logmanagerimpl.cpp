...
LogListenerPtr LogManagerImpl::createListener()
{
  ...
  boost::shared_ptr<LogListenerImpl> ptr =
      boost::make_shared<LogListenerImpl>(boost::ref(*this), boost::bind(&LogManagerImpl::pushBacklog, this, _1));
...
{
  qiLogWarning("LogManager")
      << "LogManager::getListener is deprecated. Please use LogManager::createListener instead.";
  return createListener();
}

void LogManagerImpl::recomputeVerbosities(qi::LogLevel from, qi::LogLevel to)
{
...

  {
    ...
    if (newMax != _maxLevel)
    {
      ...
      {
        providersIt->second.async<void>("setLevel", _maxLevel)
            .connect(&LogManagerImpl::providerCallback, this, _1, providersIt->first);
      }
    }
  }
}
...
void LogManagerImpl::gcProviders()
{
  ...
  for (invalidProviderIdsIt = _invalidProviderIds.begin(); invalidProviderIdsIt != _invalidProviderIds.end();
...
void LogManagerImpl::recomputeCategories()
{
  ...
  FilterMap map;
  {
    ...
    if (_listeners.size() == 1)
    {
      ...
      if (boost::shared_ptr<LogListenerImpl> l = _listeners.front().lock())
      {
        ...
        for (providersIt = _providers.begin(); providersIt != _providers.end(); ++providersIt)
        {
          providersIt->second.async<void>("setFilters", _filters)
              .connect(&LogManagerImpl::providerCallback, this, _1, providersIt->first);
        }
      }
      else
    ...
    for (unsigned int listenerIt = 0; listenerIt < _listeners.size();)
    {
      ...
      if (boost::shared_ptr<LogListenerImpl> l = _listeners[listenerIt].lock())
      {
        remove = false;
        FilterMap listenerFilters = l->filters();
        for (FilterMap::iterator it = listenerFilters.begin(); it != listenerFilters.end(); ++it)
        {
  ...
  for (FilterMap::iterator it = map.begin(); it != map.end();)
  {
    ...
    {
      if (it2->first.find('*') != it2->first.npos && qi::os::fnmatch(it2->first, it->first) &&
